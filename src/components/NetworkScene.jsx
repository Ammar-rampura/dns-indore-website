  ...
  useEffect(() => {
    ...
    import('three')
      .then((THREE) => {
        ...
        const hubMat = new THREE.MeshBasicMaterial({ color: 0x3b8fd9 })
        const hubMeshes = hubs.map((p, i) => {
          const m = new THREE.Mesh(hubGeo, i % 5 === 0 ? new THREE.MeshBasicMaterial({ color: 0xe8b84b }) : hubMat)
          m.position.copy(p)
          group.add(m)
          return m
        })
        ...
        const cGold = new THREE.Color(0xe8b84b)
        dots.forEach((d, i) => {
